...
(defun direction (cell neighbor)
  "Returns the relative direction of neighbor to cell."
  (let ((row-cel (first cell)) ;
        (col-cel (second cell))
        (row-nei (first neighbor))
        (col-nei (second neighbor)))
    (cond
      ((and (= col-cel col-nei) (= row-cel (1+ row-nei))) ; cel row is one above neighbor row, making neighbor 'above'/north
       :north)
      ((and (= col-cel col-nei) (= row-cel (1- row-nei))) ; neighbor is below
       :south)
      ((and (= row-cel row-nei) (= col-cel (1+ col-nei))) ; neighbor is to the west
       :west)
      ((and (= row-cel row-nei) (= col-cel (1- col-nei))) ; neighbor is to the east
       :east))))
