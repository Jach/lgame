...
(defmethod initialize-instance :after ((self fist) &key)
  (let ((image (lgame:get-texture lgame:*texture-loader* :fist :color-key '(0 0 0))))
    (setf (lgame:image-of self) image)
    (setf (lgame:rect-of self) (lgame:get-texture-rect image))))

(defmethod lgame:update ((self fist))
  "Move the fist based on mouse position"
  (with-accessors ((rect lgame:rect-of) (punching? .punching?)) self
    (multiple-value-bind (x y) (sdl2:mouse-state)
      (lgame:set-rect rect :x x :y y)
      (lgame:move-rect rect (/ (sdl2:rect-width rect) -2) 0))
    (when punching?
      (lgame:move-rect rect 5 10))))
...
(defmethod punch ((self fist) target)
  ...
  (unless (.punching? self)
    ...
    (lgame:with-inflated-rect (hitbox (lgame:rect-of self) -5 -5)
      (lgame:collide-rect? (lgame:rect-of target) hitbox))))
...
(defmethod initialize-instance :after ((self chimp) &key)
  (let ((image (lgame:get-texture lgame:*texture-loader* :chimp :color-key '(255 0 0))))
    (setf (lgame:image-of self) image)
    (setf (lgame:rect-of self) (lgame:get-texture-rect image))
    (lgame:move-rect (lgame:rect-of self) 10 10)))

(defmethod lgame:update ((self chimp))
  (if (zerop (lgame:angle-of self))
      (walk self)
      (spin self)))

(defmethod walk ((self chimp))
  "Moves monkey back and forth across the screen"
  (with-accessors ((rect lgame:rect-of) (speed .move) (flip lgame:flip-of)) self
    (lgame:move-rect rect speed 0)
    (when (or (<= (lgame:rect-left rect) 0) (>= (lgame:rect-right rect) (lgame:rect-right lgame:*screen-rect*)))
      (setf speed (- speed))
      (lgame:move-rect rect speed 0)
...
(defmethod spin ((self chimp))
  "Spin monkey image"
  (incf (lgame:angle-of self) 12)
  (if (>= (lgame:angle-of self) 360) ; complete rotation
      (setf (lgame:angle-of self) 0.0d0)))

(defmethod punched ((self chimp))
  "Cause the monkey to start spinning"
  (if (zerop (lgame:angle-of self))
      (setf (lgame:angle-of self) 1)))

(defun main ()
  (lgame:init)
  (lgame:create-texture-loader *main-dir*)
  (lgame:create-window "Monkey Fever" lgame::+sdl-windowpos-centered+ lgame::+sdl-windowpos-centered+ 468 60)
  (lgame:create-renderer)
  (sdl2:hide-cursor)

  (let* ((font (lgame:load-font (lgame:get-default-font) 20))
         (banner-txt (lgame:render-text font "Pummel The Chimp, And Win $$$" 10 10 10))
         (banner-txt-rect (lgame:get-texture-rect banner-txt))

         (whiff-sound (load-sound "whiff.wav"))
         (punch-sound (load-sound "punch.wav"))
         (chimp (make-instance 'chimp))
         (fist (make-instance 'fist))
         (allsprites (make-instance 'lgame:group :sprites (list fist chimp))))

    (lgame:move-rect banner-txt-rect (- (/ 468 2) (/ (sdl2:rect-width banner-txt-rect) 2)) 1) ; no centerx/centery construction yet
...
(defun game-tick (banner-txt banner-txt-rect whiff-sound punch-sound chimp fist allsprites)
  (lgame:do-event (event)
    (if (or
          (= (lgame:event-type event) lgame::+sdl-quit+)
          (and (= (lgame:event-type event) lgame::+sdl-keydown+)
               (= (lgame:key-scancode event) lgame::+sdl-scancode-escape+)))
        (setf *running?* nil))
    (if (= (lgame:event-type event) lgame::+sdl-mousebuttondown+)
        (if (punch fist chimp)
            (progn (sdl2-mixer:play-channel -1 punch-sound 0)
                   (punched chimp))
            (sdl2-mixer:play-channel -1 whiff-sound 0)))
    (if (= (lgame:event-type event) lgame::+sdl-mousebuttonup+)
        (unpunch fist)))

  (lgame:update allsprites)
